...
def add_oblong(layer: Layer, p1: Point, p2: Point, radius: float, color: Rgba, thickness: float, fill: Rgba, text: str):
    ...
    polygon = layer.add_freeform_polygon(startPoint, points, color=color, fill=fill, line_thickness=thickness)
    if text:
        polygon.name = text
        ...
        layer.add_text_box(text_point, text, angle=angle, font_size=16, border_thickness=0, color=color, fill=Rgba(0,0,0,0))
    return polygon